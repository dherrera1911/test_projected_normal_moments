...
from vector_constraint import make_basis, ParametrizedVector
...
def main(dimension='3d'):
    ...
    if dimension=='3d':
        N_DIM_LIST = [3]
        SAVING_DIR = saving_dirs['moment_match_ellipse_param_3d']
    elif dimension=='nd':
        N_DIM_LIST = config['simulation_parameters']['n_dim_list']
        SAVING_DIR = saving_dirs['moment_match_ellipse_param_nd']

    # Remove N_DIM_LIST elements smaller or equal than n_basis
    N_DIM_LIST = [n_dim for n_dim in N_DIM_LIST if n_dim > N_BASIS]
    ...
    for n_dim in N_DIM_LIST:
        ...
        B_basis = make_basis(n_dim=n_dim, n_basis=N_BASIS)
        ...
        for v, sigma_scale in enumerate(SIGMA_LIST):
            ...
            while r < N_SIMULATIONS:
                ...
                # Sample B vectors
                basis_coefs = param_sampling.make_ortho_vectors(N_BASIS, N_DIRS)
                results['B_vecs'][v][r] = torch.einsum(
                  'ij,jk->ik', basis_coefs, B_basis
                ).to(dtype=DTYPE)
                ...
                parametrize.remove_parametrizations(
                  prnorm.ellipse, "sqrt_vecs"
                ) # Remove previous parametrization
                parametrize.register_parametrization(
                  prnorm.ellipse, "sqrt_vecs", ParametrizedVector(n_dim=n_dim, n_basis=N_BASIS)
                )
                prnorm.ellipse.sqrt_vecs = vec_init
