    ...
    for n, nDim in enumerate(nDimVec):
        ...
        for v, varScaleUn in enumerate(varScaleVec):
            ...
            while r < nReps:
                ...
                print(progressStr)
                # Get parameters
                muTrue[v, :, r], covTrue[v, :, :, r] = sample_parameters(
                    nDim, covType=covType
                )
                ...
                prnormFit = pn.ProjNorm(
                    nDim=nDim, muInit=muInit, covInit=covInit, dtype=dtype
                )
                loss = prnormFit.moment_match(
                    muObs=gammaTrue[v, :, r],
                    covObs=psiTrue[v, :, :, r],
                    nIter=nIter,
                    lr=lr,
                ...
                # Check that the fitted mu is not nan
                if not torch.isnan(prnormFit.mu).any():
                    # Get the fitted moments without gradient
                    with torch.no_grad():
                        gammaTaylor[v, :, r], psiTaylor[v, :, :, r] = (
                            prnormFit.get_moments()
                        )
                        gammaApprox[v, :, r], psiApprox[v, :, :, r] = (
                            prnormFit.empirical_moments(nSamples=nSamples)
                        )  # Empirical
